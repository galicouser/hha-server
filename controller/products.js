...
async function editProductData(req, res) {
    try {
        const { id, validity, price,master_id } = req.body;
        ...
        // Find the corresponding ProductDetails by master_id
        const productDetails = await ProductDetails.findOne({ master_id: master_id });
        if (!productDetails) {
            return res.status(404).json({ result:"error",message: 'Plan not found' });
        }
        // const metadataItem = productDetails.metadata.find(metadata => metadata._id === id);
        const metadataItem = productDetails.metadata.find(
            (metadata) => metadata._id.toString() === id
          );
        if (!metadataItem) {
            return res.status(404).json({ result:"error",message: 'Plan details not found' });
        }


        metadataItem.price = price;
        metadataItem.pricedesc = `$${price}/mo for ${validity} ${validity > 1 ? 'months' : 'month'}`;;
        // productDetails.date_availed = new Date();
        // productDetails.date_of_expiry = currentDate;
        const newProductMasterSaved =  productDetails.save();
        return res.status(200).json({ result:"success", message: 'Plan price updated',data:productDetails.metadata });
    } catch (err) {
        console.error(err);
